...
const $$Version = (input) => {
  ...
  let assets = Decoder.field(`assets`, Decoder.array($$Asset))(input)
...
const $Stores_Try = new (class extends Store {
  ...
  get compiling () {
    if (this.props.compiling != undefined) {
  ...
  init() {
    return (this.initialized ? null : (async () => {
     ...
     await (async ()=> {
      try {
        return await $AssetLoader.loadStyle(`/codemirror.neo.min.css`)
      } catch(_error) {
  ...
  setSource(source) {
    return new Promise((_resolve) => {
      this.setState(_update(this.state, { source: source }), _resolve)
    })
  }
  ...
  compile() {
    ...
    let response = await (async ()=> {
     ...
     await (_compare(response.status, 500) ? new Promise((_resolve) => {
      this.setState(_update(this.state, { src: $Stores_Try.createObjectUrl(response.body, `text/html`) }), _resolve)
    }) : (async () => {
      try {
        let url = await $Stores_Try.createObjectUrl(response.body, `application/javascript`)

     await new Promise((_resolve) => {
      this.setState(_update(this.state, { src: $Stores_Try.createObjectUrl($Stores_Try.html(url), `text/html`) }), _resolve)
    })
      }
      catch(_error) {
        if (_error instanceof DoError) {
    ...
    new Promise((_resolve) => {
      this.setState(_update(this.state, { compiling: false }), _resolve)
    })
    }
    })()
  }
...
const $Versions = new (class extends Store {
  ...
  load() {
    ...
    let _3 = Decoder.array($$Version)(object)
...
class $Pages_Try extends Component {
  ...
  handleChange(value) {
    return this.setSource.bind(this)(value)
  }
  ...
  render() {
    return (this.initialized ? _createElement("div", {
      ...
      className: `pages-try-file`,
      style: {

      }
    }, [`Main.mint`]), _createElement("button", {
      "onClick": (event => (this.handleCompile.bind(this))(_normalizeEvent(event))),
...
class $Install extends Component {
  ...
  render() {
    return _createElement($Page, {  }, _array(_createElement($Title, {  }, _array(`Install`)), _createElement($SubTitle, {  }, _array(`Follow the directions below to install Mint:`)), _createElement("ol", {
