...
const $Stores_Try = new (class extends Store {
  ...
  init(url) {
    return $Stores_Try.loadSource.bind($Stores_Try)(url)
  }
  ...
  loadSource(url) {
    return (async () => {
      ...
      _result = await $Stores_Try.compile.bind($Stores_Try)()
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
  ...
  compile() {
    return (async () => {
      ...
      _result = await (_compare(response.status, 500) ? new Promise((_resolve) => {
        this.setState(_update(this.state, new Record({
      src: $Stores_Try.createObjectUrl.bind($Stores_Try)(response.body, `text/html`)
      })), _resolve)
      }) : (async () => {
        let _result = null;
        ...
        let newSrc = await $Stores_Try.createObjectUrl.bind($Stores_Try)($Stores_Try.html.bind($Stores_Try)(url), `text/html`)
...
const $Stores_Versions = new (class extends Store {
  ...
  refresh() {
    return (this.initialized ? $Promise.never() : $Stores_Versions.load.bind($Stores_Versions)())
  }
...
class $Showcase_Highlight extends Component {
  ...
  handleClick(event) {
    return (async () => {
      ...
      _result = await this.setActive.bind(this)(this.name)
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
  ...
  handleMouseEnter(event) {
    return (async () => {
      ...
      _result = await this.setOver.bind(this)(this.name)
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
...
class $Pages_Try extends Component {
  ...
  render() {
    ...
    }, [_createElement($CodeMirror, { "javascripts": [`/codemirror.min.js`], "styles": [`/codemirror.min.css`, `/codemirror.neo.min.css`], "onChange": this.handleChange.bind(this), "value": this.source, "theme": `neo` })])]), this.frame])
  }
}
...
class $CodeMirror extends Component {
  ...
  componentDidMount() {
    return (async () => {
      ...
      try {
         await $AssetLoader.loadAll($AssetLoader.loadScript.bind($AssetLoader), this.javascripts)

       await $AssetLoader.loadAll($AssetLoader.loadStyle.bind($AssetLoader), this.styles)

      _result = await this.initializeEditor.bind(this)()
      }
      catch (_error) {
        if (_error instanceof DoError) {} else {
            console.warn(`Unhandled error in sequence expression:`)
  ...
  render() {
    return [_createElement("textarea", {
      "ref": (ref => { ref ? this.saveReference.bind(this).call(this, ref) : null }),
