...
const $Stores_Try = new (class extends Store {
    ...
    super()
    this.props = {
        compiling: false,error: ``,source: ``,src: ``
    }
  }
  ...
  compile() {
    ...
    let response = await (async ()=> {
     ...
     await (_compare(response.status, 500) ? new Promise((_resolve) => {
      this.setState(_update(this.state, { src: $Stores_Try.createObjectUrl.bind($Stores_Try)(response.body, `text/html`) }), _resolve)
    }) : (async () => {
      try {
...
$CodeMirror.defaultProps = {
  javascripts: [`https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.39.0/codemirror.min.js`],styles: [`https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.39.0/codemirror.min.css`],onChange: (((value) => {
  return null
  })),loadingContent: $Html.empty(),lineNumbers: true,value: ``,theme: ``,mode: ``
}
