...
class $Showcase extends Component {
  ...
  render() {
    ...
    }), _createElement($Showcase_HighlightBlock, { "line": `fun increment : Void`, "name": `function` }, _array(_createElement($Showcase_Highlight, { "text": `next { counter = counter + 1 }`, "name": `next` }))), _createElement("div", {
      className: `showcase-spacer`
    }), _createElement($Showcase_Block, { "line": `fun decrement : Void` }, _array(`next { counter = counter - 1 }`)))), _createElement("div", {
      className: `showcase-spacer`
...
class $CodeMirror extends Component {
  ...
  componentDidMount() {
    return (async () => {
      try {
         await (async ()=> {
      try {
        return await $AssetLoader.loadAll($AssetLoader.loadScript.bind($AssetLoader), this.javascripts)
      } catch(_error) {
     ...
     await (async ()=> {
      try {
        return await $AssetLoader.loadAll($AssetLoader.loadStyle.bind($AssetLoader), this.styles)
      } catch(_error) {
     ...
     await this.initializeEditor.bind(this)()
      }
      catch(_error) {
        if (_error instanceof DoError) {
        } else {
  ...
  render() {
    ...
    }), (this.editor ? $Html.empty() : this.loadingContent)]
  }
}
